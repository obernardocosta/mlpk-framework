...
_date_fmt = '%Y-%m-%d'
_start_date, _end_date = map(lambda x: dt.datetime.strptime(
    x, _date_fmt), ['2013-08-19', '2016-07-19'])
...
_tmp_range = pd.date_range(
    start=_start_date, periods=_90d_periods + 1, freq='90D')

# making the list of `_start_date _end_date`, strf separated by a space
_rolling_dates = [' '.join(map(lambda x: x.strftime(_date_fmt), [_tmp_range[i], _tmp_range[i+1]])
                           )
...
_pytrends.build_payload(
    _kw_list, cat=_cat, timeframe=_dates, geo=_geo, gprop=_gprop)
...
for _dates in _rolling_dates[1:]:
    ...
    _df_trends = (pd.concat([_df_trends,
                  ...
                  # making sure that we have one value per day.
                  .resample('D', closed='right').bfill()
                  )
